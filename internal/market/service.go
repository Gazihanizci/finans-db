...
func (s *Service) fetch(ctx context.Context) (Snapshot, error) {
	...
	usdtry, eurtry, gbptry, rateSource, err := s.fetchRates(ctx)
	if err != nil {
		return Snapshot{}, err
	}
...
func (s *Service) fetchRates(ctx context.Context) (usdtry float64, eurtry float64, gbptry float64, source string, err error) {
	var lastErr error
	for _, p := range s.ratesProviders {
		u, e, g, eErr := p.GetRates(ctx)
		if eErr == nil {
			return u, e, g, p.Name(), nil
		}
		lastErr = eErr
	}
	if lastErr == nil {
